...
def returns(returns, benchmark=None,
            ...
            subtitle=True, savefig=None, show=True,
            prepare_returns=True, data=True):
    ...
    if not show:
        return fig
    if data:
        return [(returns.add(1).cumprod() - 1)*100, (benchmark.add(1).cumprod() - 1)*100]
...
def yearly_returns(returns, benchmark=None,
    ...
    if not show:
        return fig
    if data:
        return [returns*100, benchmark*100]
...
def histogram(returns, resample='M', fontname='Arial',
    ...
    if resample == 'W':
        ...
        title = ""
    fig = _core.plot_histogram(returns,
                               resample=resample,
                               grayscale=grayscale,
                               fontname=fontname,
...
def drawdown(returns, grayscale=False, figsize=(10, 5),
             ...
             resample=None, subtitle=True, savefig=None, show=True, data=True):
...
def rolling_volatility(returns, benchmark=None,
                       ...
                       subtitle=True, savefig=None, show=True, data=True,):
